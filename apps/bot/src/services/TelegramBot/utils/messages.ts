export const getWelcomeMessage = (username: string): string => {
  let message = `Hello <b>${username === "User" ? "" : "@"}${username}</b>👋!  Welcome to Wallet Buddy Bot. \n\n`
...
export const getAboutMessage = (username: string): string => {
  let message = `Hello <b>${username === "User" ? "" : "@"}${username}</b>👋! Thank you for choosing Wallet Buddy Bot 🙏. \n\n`
